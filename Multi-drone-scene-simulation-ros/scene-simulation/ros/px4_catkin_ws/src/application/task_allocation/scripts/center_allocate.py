...
def wait_bid_price(uav_num,task, EnvTemp,EnvRain,EnvWind):
    ...
    for i in range(uav_num):
        ...
        try:
            bid_info = rospy.ServiceProxy('/uav'+str(i)+'/bid_price',Bid_Info)

            resp = bid_info(task,EnvTemp,EnvRain,EnvWind) 
            price_list[resp.uav_id] = resp.bid_price        # 此处将price_list索引认为是无人机id，下同
...
if __name__ == '__main__':
    ...
    rospy.Subscriber('/all_task_allocate',Task_List,task_list_handle)             ## 接收待任务信息  
    bid_result_pub = rospy.Publisher('/bid_result',Bid_Result,queue_size=1)  # 给投标价格最高的无人机发送
    ...
    allocated_task_state_pub = rospy.Publisher('/allocated_task_state',TASK_State,queue_size=1)
        ...
        if len(task_list)!=0 and len(UAVs)>=2:
            for task in task_list:
                if task.done == 0: # 存在任务未分配    0表示未分配 1表示已分配  2表示无无人机匹配 3表示正在执行 4表示执行完毕
                    rospy.loginfo("task {} is being allocated: uav_num:{}".format(task.task_id,len(UAVs)))
                    # task_msg = task_to_msg(task)
                    #bid_task_pub.publish(task)
                    
                    price_list,index_list,St_list,Et_list = wait_bid_price(uav_num,task,EnvTemp,EnvRain,EnvWind)  # 等待每个无人机出价  出价包括插入的位置索引和出价格
                    # 获取最大价格    
                    ...
                    if index_list[max_uav]!= -1:  # 任务满足约束 具有可插入位置 更新本地任务状态
                        ...
                        allocated_task_state_pub.publish(task)  # 将分配完成的任务结果发回task_management
                    else:
