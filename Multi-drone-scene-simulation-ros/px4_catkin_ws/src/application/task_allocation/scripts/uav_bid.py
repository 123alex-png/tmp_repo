...
def resource_cost_cal(Pg, EnterPoint, LeavePoint, SR_max, Seq, Pu, maxV, maxTF,
                      TF):
    """
    计算无人机资源约束的任务成本
    ...
    """
    # 计算通信距离成本  对任务插入位置无要求
    if cal_ED_distance(Pg, EnterPoint) <= SR_max and cal_ED_distance(
            Pg, LeavePoint) <= SR_max:
        Cost_signal = 0
    else:
    ...
    print(Cost_signal)
    # 计算续航时间成本  对任务插入位置有要求
    Flength_line = cal_ED_distance(Pu, [
        Seq[0].EnterPoint.way_point_pos.x, Seq[0].EnterPoint.way_point_pos.y,
        Seq[0].EnterPoint.way_point_pos.z
    ])
    for i in range(1, len(Seq) - 1):
        Flength_line += cal_ED_distance([
            Seq[i].LeavePoint.way_point_pos.x,
            Seq[i].LeavePoint.way_point_pos.y,
            Seq[i].LeavePoint.way_point_pos.z
        ], [
            Seq[i + 1].EnterPoint.way_point_pos.x,
            Seq[i + 1].EnterPoint.way_point_pos.y,
            Seq[i + 1].EnterPoint.way_point_pos.z
        ])
...
def timewindow_cost_cal(E_tk, S_tk1, preT_k, sufTk1, Seq, insert_index):
    Seq[insert_index].Ts_l = Seq[insert_index].Te - Seq[
        insert_index].execute_time
    Seq[insert_index].Te_e = Seq[insert_index].Ts + Seq[
        insert_index].execute_time
    # 四种情况
    # 1
    if E_tk <= Seq[insert_index].Ts and Seq[insert_index].Te <= S_tk1 and Seq[
            insert_index].execute_time < S_tk1 - E_tk:
        if Seq[insert_index].E_t + sufTk1 <= S_tk1 and Seq[
                insert_index].S_t >= Seq[insert_index].Ts and Seq[
                    insert_index].S_t <= Seq[insert_index].Ts_l:
            Seq[insert_index].S_t = Seq[insert_index].S_t
        elif Seq[insert_index].Ts >= Seq[
                insert_index].S_t and Seq[insert_index].Te_e + sufTk1 <= S_tk1:
            Seq[insert_index].S_t = Seq[insert_index].Ts
        else:
            Seq[insert_index].S_t = Seq[insert_index].Ts - 1
        Seq[insert_index].E_t = Seq[insert_index].S_t + Seq[
            insert_index].execute_time
    elif E_tk <= Seq[insert_index].Ts and Seq[
            insert_index].Ts <= S_tk1 and Seq[insert_index].Te >= S_tk1 and Seq[
                insert_index].execute_time < S_tk1 - Seq[insert_index].Ts:
        if Seq[insert_index].Ts > Seq[
                insert_index].S_t and Seq[insert_index].Te_e + sufTk1 < S_tk1:
            Seq[insert_index].E_t = Seq[insert_index].Te_e
        elif Seq[insert_index].E_t + sufTk1 <= S_tk1 and Seq[
                insert_index].S_t <= Seq[insert_index].Ts_l:
            Seq[insert_index].E_t = Seq[insert_index].E_t
        else:
            Seq[insert_index].E_t = Seq[insert_index].Te_e + 1
        Seq[insert_index].S_t = Seq[insert_index].E_t - Seq[
            insert_index].execute_time
    elif E_tk >= Seq[insert_index].Ts and Seq[
            insert_index].Te <= S_tk1 and Seq[insert_index].Te >= E_tk and Seq[
                insert_index].execute_time < Seq[insert_index].Te - E_tk:
        if Seq[insert_index].Ts_l > Seq[
                insert_index].S_t and Seq[insert_index].E_t + sufTk1 < S_tk1:
            Seq[insert_index].S_t = Seq[insert_index].S_t
        else:
            Seq[insert_index].S_t = Seq[insert_index].Ts - 1
        Seq[insert_index].E_t = Seq[insert_index].S_t + Seq[
            insert_index].execute_time
    elif (E_tk >= Seq[insert_index].Ts or Seq[insert_index].Te
          >= S_tk1) and Seq[insert_index].execute_time >= min(
              Seq[insert_index].Te, S_tk1) - max(Seq[insert_index].Ts, E_tk):
        Seq[insert_index].S_t = Seq[insert_index].Ts - 1
        Seq[insert_index].E_t = Seq[insert_index].Te + 1
    return Seq[insert_index].S_t, Seq[insert_index].E_t


def task_cost_cal(TaskReqUav, TaskReqPayload, UavDomain, PayloadType, Seq,
                  insert_index, maxV, pos, max_time):
    """
    任务约束成本计算
    ...
    if len(Seq) == 1:
        E_tk = 0
        S_tk1 = max_time
        preT_k = cal_MD_distance(pos, [
            Seq[insert_index].EnterPoint.way_point_pos.x,
            Seq[insert_index].EnterPoint.way_point_pos.y,
            Seq[insert_index].EnterPoint.way_point_pos.z
        ]) / maxV
        sufTk1 = 0
        ...
        Seq[insert_index].E_t = Seq[insert_index].S_t + Seq[
            insert_index].execute_time
        # update
        Seq[insert_index].S_t, Seq[insert_index].E_t = timewindow_cost_cal(
            E_tk, S_tk1, preT_k, sufTk1, Seq, insert_index)
    else:
        if insert_index == 0:  # 插入第一个位置
            ...
            sufTk1 = cal_MD_distance([
                Seq[insert_index].LeavePoint.way_point_pos.x,
                Seq[insert_index].LeavePoint.way_point_pos.y,
                Seq[insert_index].LeavePoint.way_point_pos.z
            ], [
                Seq[insert_index + 1].EnterPoint.way_point_pos.x,
                Seq[insert_index + 1].EnterPoint.way_point_pos.y,
                Seq[insert_index + 1].EnterPoint.way_point_pos.z
            ]) / maxV
            Seq[insert_index].S_t = E_tk + preT_k
            Seq[insert_index].E_t = Seq[insert_index].S_t + Seq[
                insert_index].execute_time
            ...
            S_tk1 = max_time
            preT_k = cal_MD_distance([
                Seq[insert_index - 1].LeavePoint.way_point_pos.x,
                Seq[insert_index - 1].LeavePoint.way_point_pos.y,
                Seq[insert_index - 1].LeavePoint.way_point_pos.z
            ], [
                Seq[insert_index].EnterPoint.way_point_pos.x,
                Seq[insert_index].EnterPoint.way_point_pos.y,
                Seq[insert_index].EnterPoint.way_point_pos.z
    ...
    if Seq[insert_index].S_t >= Seq[insert_index].Ts and Seq[
            insert_index].S_t <= Seq[insert_index].Te and Seq[
                insert_index].E_t >= Seq[insert_index].Ts and Seq[
                    insert_index].E_t <= Seq[insert_index].Te:
        Cost_time = 0
    else:
        Cost_time = Max_Value
    print("Cost_time")
...
def bid_price(uav, task, EnvWind, EnvTemp, EnvRain):

    Cost_E = env_cost_cal(
        EnvWind, uav.max_WR, EnvTemp, uav.work_temperature_min,
        uav.work_temperature_max, EnvRain,
        uav.wl)  # env_cost_cal(s_wind,WR_max,cur_OT,min_OT,max_OT,cur_WL,WL):
    print("Cost_E")
    print(Cost_E)
    if Cost_E >= Max_Value:
        price = task.R - Cost_E
    ...
    for i in range(0, len(uav_accepted) + 1):
        ...
        Cost_R = resource_cost_cal(
            [uav.ground_station.x, uav.ground_station.y, uav.ground_station.z],
            [
                task.EnterPoint.way_point_pos.x,
                task.EnterPoint.way_point_pos.y,
                task.EnterPoint.way_point_pos.z
            ], [
                task.LeavePoint.way_point_pos.x,
                task.LeavePoint.way_point_pos.y,
                task.LeavePoint.way_point_pos.z
            ], uav.max_comm_length, temp_accepted,
            [uav.position.x, uav.position.y, uav.position.z],
            uav.velocity_size, uav.power, uav.flight_time)
        print("Cost_R")
        print(Cost_R)
        ...
        if Cost_R >= Max_Value:
            ...
            continue
        Cost_T1, all_insert_S_t[i], all_insert_E_t[i] = task_cost_cal(
            task.req_uav_type, task.req_payload_type, uav.uav_type,
            uav.payload_type, temp_accepted, i, uav.velocity_size,
            [uav.position.x, uav.position.y, uav.position.z], 9999)
        #print(f"对任务{task.id},无人机{uav.id}任务所需无人机资源与时间窗约束成本：{Cost_T1}")
        if Cost_T1 >= Max_Value:
            all_insert_Cost[i] = Cost_T1
            continue
        Cost_T2 = dist_cost_cal(
            [uav.position.x, uav.position.y, uav.position.z], temp_accepted, i)
    ...
    #判断是否满足时序关系
    #if task.task_id == 1:
    # 	if task.S_t > seq_E_t and task.S_t < task.E_t + margin:
    #		print("不能满足时序关系")
    #	else:
    #		return uav.uav_id, price, -1,task.S_t,task.E_t
    if min_Cost - 10 >= Max_Value:
        ...
        print(
            f"对任务{task.task_id},无人机{uav.uav_id}插入在位置{min_insert_index}最佳，price：{price}"
        )
...
def bid_price_handle(req):
    ...
    uav_id, price, index, task_msg.S_t, task_msg.E_t = bid_price(
        uav_msg, req.task, req.EnvWind, req.EnvTemp, req.EnvRain)
...
def start_view_point_plan_handle(start_msg):
    ...
    if start_msg.uav_id == uav_id:
        ...
        for task in accepted:
            rospy.wait_for_service('/uav' + str(uav_id) +
                                   '/view_point_plan_service')
            try:
                cal_task_path = rospy.ServiceProxy(
                    '/uav' + str(uav_id) + '/view_point_plan_service',
                    View_Plan)
...
def start_execute_task_handle(execute_msg):
    ...
    if execute_msg.uav_id == uav_id:
        for task in accepted:
            if len(task.task_path
                   ) == 0:  # 任务路径若有模型计算则有实际路径  否则有Enteroint和Leaveoint两点
...
if __name__ == '__main__':
    ...
    rospy.Service('/uav' + str(uav_id) + '/bid_price', Bid_Info,
                  bid_price_handle)  # /uav0/bid_price
    rospy.Subscriber('/bid_result', Bid_Result, bid_result_handle)
    rospy.Subscriber('/start_view_point_plan', UAV_State,
                     start_view_point_plan_handle)
    rospy.Subscriber('/start_execute_task', UAV_State,
                     start_execute_task_handle)
    # start_view_point_plan_pub = rospy.Publisher('/uav'+str(uav_id)+'/view_point_plan',Int8,queue_size=10) # 向viewpointplan发送任务的id
    uav_tasks_pub = rospy.Publisher('/uav' + str(uav_id) + '/uav_tasks',
                                    UAV_Tasks,
                                    queue_size=10)
