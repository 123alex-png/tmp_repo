...
class collision_check:
    ...
    def checkdimdfs(self, segment, cli, axis=0):
        ...
        if axis == 3:
            ...
            a, b = linear_equation(segment[0], segment[3], segment[1],
                                   segment[4])
            c, d = linear_equation(segment[1], segment[4], segment[2],
                                   segment[5])
            for bindex in cli:
                if self.collision_cube_line(bindex, segment, [a, b, c, d]):
                    print("the collision happens where segment = " +
                          str(segment) + ", and block index is:" + str(bindex))
                    return True
            return False
        tonext = []
        for seg in cli:
            if self.intersection(
                [self.block[seg][axis], self.block[seg][axis + 3]],
                [segment[axis], segment[axis + 3]]):
                tonext.append(seg)
    ...
    def collision_cube_line(self, cindex, segment, para):
        ...
        commonx = self.intersection([cube[0], cube[3]],
                                    [segment[0], segment[3]])
        if not commonx:
            return False
