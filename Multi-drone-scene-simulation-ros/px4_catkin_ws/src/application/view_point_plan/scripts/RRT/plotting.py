...
class Plot(object):
    ...
    def plot_tree_2d(self, trees):
        ...
        for i, tree in enumerate(trees):
            for start, end in tree.E.items():
                if end is not None:
                    trace = go.Scatter(x=[start[0], end[0]],
                                       y=[start[1], end[1]],
    ...
    def plot_tree_3d(self, trees):
        ...
        for i, tree in enumerate(trees):
            for start, end in tree.E.items():
                if end is not None:
                    trace = go.Scatter3d(x=[start[0], end[0]],
                                         y=[start[1], end[1]],
                                         z=[start[2], end[2]],
                                         line=dict(color=colors[i]),
                                         mode="lines")
    ...
    def plot_obstacles(self, X, O):
        ...
        if X.dimensions == 2:  # plot in 2D
            ...
            for O_i in O:
                obs = go.Mesh3d(x=[
                    O_i[0], O_i[0], O_i[3], O_i[3], O_i[0], O_i[0], O_i[3],
                    O_i[3]
                ],
                                y=[
                                    O_i[1], O_i[4], O_i[4], O_i[1], O_i[1],
                                    O_i[4], O_i[4], O_i[1]
                                ],
                                z=[
                                    O_i[2], O_i[2], O_i[2], O_i[2], O_i[5],
                                    O_i[5], O_i[5], O_i[5]
                                ],
    ...
    def plot_min_obstacles(self, X, O):
        """
            Plot min obstacles
            :param X: Search Space
            :param O: list of obstacles
        ...
        if X.dimensions == 2:  # plot in 2D
            ...
            for O_i in O:
                obs = go.Mesh3d(
                    # [ 0,1,2,3,4,5,6,7]
                    x=[
                        O_i[0][0], O_i[1][0], O_i[2][0], O_i[3][0], O_i[4][0],
                        O_i[5][0], O_i[6][0], O_i[7][0]
                    ],
                    y=[
                        O_i[0][1], O_i[1][1], O_i[2][1], O_i[3][1], O_i[4][1],
                        O_i[5][1], O_i[6][1], O_i[7][1]
                    ],
                    z=[
                        O_i[0][2], O_i[1][2], O_i[2][2], O_i[3][2], O_i[4][2],
                        O_i[5][2], O_i[6][2], O_i[7][2]
                    ],
                    i=[7, 0, 0, 0, 4, 4, 6, 6, 4, 0, 3, 2],
                    j=[3, 4, 1, 2, 5, 6, 5, 2, 0, 1, 6, 3],
                    k=[0, 7, 2, 3, 6, 7, 1, 1, 5, 5, 7, 6],
