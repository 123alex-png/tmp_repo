...
class RRTBase(object):
    ...
    def new_and_near(self, tree, q):
        ...
        """
        x_rand = self.X.sample_free()  # 随机生成点
        x_nearest = self.get_nearest(tree, x_rand)  # 求最近的点
        x_new = self.bound_point(steer(x_nearest, x_rand, q[0]))
        # check if new point is in X_free and not already in V
        if not self.trees[0].V.count(x_new) == 0 or not self.X.obstacle_free(
                x_new):
            return None, None
    ...
    def connect_to_point(self, tree, x_a, x_b):
        ...
        """
        if self.trees[tree].V.count(x_b) == 0 and self.X.collision_free(
                x_a, x_b, self.r):
    ...
    def can_connect_to_goal(self, tree):
        ...
        if self.x_goal in self.trees[tree].E and x_nearest in self.trees[
                tree].E[self.x_goal]:
            # tree is already connected to goal using nearest vertex
            return True
        if self.X.collision_free(x_nearest, self.x_goal,
                                 self.r):  # check if obstacle-free
            return True
        return False
