...
def generate_random_obstacles(X, start, end, n):
    ...
    while i < n:
        ...
        for j in range(X.dimensions):
            ...
            max_edge_length = (X.dimension_lengths[j][1] -
                               X.dimension_lengths[j][0]) / 10.0
            # None of the sides of a hyperrectangle can be higher than 0.01 of the total span
            # in that particular X.dimensions
            min_edge_length = (X.dimension_lengths[j][1] -
                               X.dimension_lengths[j][0]) / 100.0
        ...
        if len(list(
                X.obs.intersection(obstacle))) > 0 or scollision or fcollision:
            continue
        i += 1
