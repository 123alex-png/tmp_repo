...
from RRT_star_bid_h.rrt_star_bid import RRTStarBidirectional
        ...
        while True:
            for q in self.Q:  # iterate over different edge lengths
                for i in range(
                    ...
                    if self.prc and random.random(
                    ) < self.prc:  # probabilistically check if solution found
    ...
    def lazy_shortening(self):
        ...
        if self.sigma_best is not None and len(self.sigma_best) > 2:
            ...
            if self.X.collision_free(v_a, v_b, self.r):
                ...
                if v_a in self.trees[0].E and v_b in self.reconstruct_path(
                        0, self.x_init, v_a):
                    self.trees[0].E[v_a] = v_b
                elif v_a in self.trees[1].E and v_b in self.reconstruct_path(
                        1, self.x_goal, v_a):
                    self.trees[1].E[v_a] = v_b
                elif v_b in self.trees[0].E and v_a in self.reconstruct_path(
                        0, self.x_init, v_b):
                    self.trees[0].E[v_b] = v_a
                elif v_b in self.trees[1].E and v_a in self.reconstruct_path(
                        1, self.x_goal, v_b):
                ...
                self.c_best -= sum(
                    dist_between_points(i, j)
                    for i, j in pairwise(self.sigma_best[a:b + 1]))
                # add cost of new edge
                self.c_best += dist_between_points(self.sigma_best[a],
                                                   self.sigma_best[b])
