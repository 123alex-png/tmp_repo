...
def node_is_in_Obstacles(node, Obstacles, r):
    for i, obstacle in enumerate(Obstacles):
        if node[0] >= (obstacle[0] - r) and node[0] <= (
                obstacle[3] +
                r) and node[1] >= (obstacle[1] - r) and node[1] <= (
                    obstacle[4] - r) and node[2] >= (
                        obstacle[2] - r) and node[2] <= (obstacle[5] - r):
            return True
    return False


def rrt_star_bid_h_main(X_dimensions,
                        x_init,
                        x_goal,
                        Obstacles=None,
                        show=False):
    ...
    if Obstacles is not None:
        if node_is_in_Obstacles(x_init, Obstacles, r):  # 与障碍物相交
            ...
            X = SearchSpace(X_dimensions, Obstacles)
            rrt = RRTStarBidirectionalHeuristic(X, Q, x_init, x_goal,
                                                max_samples, r, prc,
                                                rewire_count)
            path = rrt.rrt_star_bid_h()
    else:
        X = SearchSpace(X_dimensions, None)
        rrt = RRTStarBidirectionalHeuristic(X, Q, x_init, x_goal, max_samples,
                                            r, prc, rewire_count)
        path = rrt.rrt_star_bid_h()
    # plot
    if show == True:
