    ...
    const uav_msgs::TASK_State::ConstPtr &msg)  // 接收任务信息
{
  ...
  if (msg->task_path.size() != 0) task.task_path = msg->task_path;
  task.path_length = msg->path_length;
  ROS_INFO(
      "id: %d , name:%s, type:%s, "
      "EnterPointpos:(%f,%f,%f),LeavePointpos:(%f,%f,%f),req_uav_type: %s",
      task.task_id, task.task_name.c_str(), task.task_type.c_str(),
      task.EnterPoint.way_point_pos.x, task.EnterPoint.way_point_pos.y,
      task.EnterPoint.way_point_pos.z, task.LeavePoint.way_point_pos.x,
      task.LeavePoint.way_point_pos.y, task.LeavePoint.way_point_pos.z,
  ...
  if (all_tasks.size() != 0) {
    for (int i = 0; i < all_tasks.size(); i++) {
      ROS_INFO(
          "id: %d , name:%s, "
          "EnterPointpos:(%f,%f,%f),LeavePointpos:(%f,%f,%f),req_uav_type: %s",
          all_tasks[i].task_id, all_tasks[i].task_name.c_str(),
          all_tasks[i].EnterPoint.way_point_pos.x,
          all_tasks[i].EnterPoint.way_point_pos.y,
          all_tasks[i].EnterPoint.way_point_pos.z,
          all_tasks[i].LeavePoint.way_point_pos.x,
          all_tasks[i].LeavePoint.way_point_pos.y,
          all_tasks[i].LeavePoint.way_point_pos.z,
...
int main(int argc, char **argv) {
  ...
  ros::Publisher management =
      nh.advertise<std_msgs::String>("/start_task_info_load", 100, true);
